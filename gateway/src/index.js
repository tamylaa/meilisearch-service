...
export default {
    async fetch(request, env) {
        ...
        try {
            ...
            const masterKey = env.MEILI_MASTER_KEY || env.MEILISEARCH_MASTER_KEY || '';
            const searchKey = env.MEILI_SEARCH_KEY || env.MEILISEARCH_SEARCH_KEY || '';
            const meilisearchHost = env.MEILISEARCH_HOST || env.MEILISEARCH_URL || '';
            let response;
...
async function handleSetup(env, masterKey, meilisearchHost) {
    try {
...
async function handleSearch(request, env, searchKey, meilisearchHost, userId) {
    let searchRequest;
    ...
    if (!searchRequest.q || typeof searchRequest.q !== 'string') {
        return new Response(JSON.stringify({ error: 'Query parameter "q" is required' }), { status: 400, headers: { 'Content-Type': 'application/json' } });
    }
...
async function handleDocumentIndex(request, env, masterKey, meilisearchHost, userId) {
...
async function handleDocumentDelete(request, env, masterKey, searchKey, meilisearchHost, userId) {
